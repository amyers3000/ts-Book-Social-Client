...
const Hero = () => {
    const { data } = useAppSelector((state) => (state.authenticate))
            ...
            <Grid container spacing={2} sx={{ mt: 10, ml: 5 }}>
                <Grid item md={6} xs={12}>
                    <Box style={{ backgroundColor: 'rgba(0,0,0,.2)' }}>
                        ...
                        {"firstName" in data ? <Typography variant="h5" color="secondary">{`Welcome ${data.firstName}, let's get reading`}</Typography> : null}
                    </Box>
